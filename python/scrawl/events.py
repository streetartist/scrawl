...
from .constants import resolve_key
...
def on_key(key, mode="pressed"):
    ...
    Args:
        key: Key name (for example ``"space"``, ``"left"``, or ``"a"``)
    ...
    """
    def decorator(func):
        if mode not in {"pressed", "released", "held"}:
            raise ValueError("key mode must be 'pressed', 'released', or 'held'")
        func._key_event = (resolve_key(key), mode)
        return func
    return decorator
