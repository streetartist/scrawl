"""
Game class - the main entry point for Scrawl games.
...
_HAS_NATIVE = NativeGame is not None
...
class Game:
    ...
    def run(self, fps: int = None, debug: bool = False, vsync: bool = True):
        ...
        if NativeGame is None:
            raise RuntimeError(
                "Scrawl's native engine is unavailable. Install scrawl-engine or "
                "run 'python -m maturin develop --release' from the Scrawl repository."
            )
